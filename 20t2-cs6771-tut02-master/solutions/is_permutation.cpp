...
// `absl::flat_hash_map` in case you decide it's necessary for Word Ladder. There are other, even
// solutions out there, so if you discovered one --- which is great! --- please spend some time
// tinkering with `absl::flat_hash_map` to get a better handle on its usage.
auto count_letters(std::string const& x) -> absl::flat_hash_map<char, int> {
   ...
   for (auto const letter : x) {
      // `absl::flat_hash_map::find` looks for a key, and returns an iterator to the key/value pair
